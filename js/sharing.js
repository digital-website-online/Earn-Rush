...
(() => {
  ...
  { id: "ref_7",  target: 7,  coins: 75000,  xp: 150 },
  ...
  async function trackIncomingReferral() {
    ...
    /* Remember the referral until signup verification */
    localStorage.setItem("earnRushPendingRef", ref);
  ...
  function renderTiers() {
    ...
    const existing =
      document.getElementById("referralTiers");
    ...
    tiers.forEach(tier => {
      const claimed =
        isClaimed(tier, state);

      const unlocked =
        referralCount >= tier.target;
    ...
    const shareBtn =
      document.getElementById("shareTaskBtn");
