/* EarnRush Missions — Simplified, Share-focused */
...
(() => {
  ...
  function render() {
    ...
    const existing = document.getElementById("shareSystemContainer");
    if (existing) existing.remove();

    const wrapper = document.createElement("div");
    wrapper.id = "shareSystemContainer";

    /* SHARE BUTTON - SIMPLE & PROMINENT */
    const shareCard = document.createElement("div");
    shareCard.className = "mission-card share-highlight";
    shareCard.innerHTML = `
      <div class="mission-icon">📤</div>
      <div class="mission-content">
        <div class="mission-title">Earn More: Share</div>
        <div class="mission-description">Get 🪙 coins when friends join</div>
        <div style="margin-top: 8px; font-size: 16px; font-weight: 900; color: var(--cyan);">
          📊 ${referralCount} friends joined
        </div>
      </div>
      <div class="mission-reward">
        <button type="button" id="shareTaskBtn" style="background: linear-gradient(135deg, #36f58b, #18c975) !important; padding: 11px 20px !important;">
          Share Now
        </button>
      </div>
    `;
    ...
    tiers.forEach(tier => {
      ...
      card.innerHTML = `
        ...
        <div class="mission-content">
          <div class="mission-title">${tier.target} Friends</div>
  ...
  window.EarnRushMissions = {
    render,
    handleTap: () => {} // Placeholder for old logic
  };
})();
